...
class KeyStoreUtils {
    ...
    fun toKeyStoreFile(
        certificates: Map<String, X509Certificate>,
        outputDir: File,
        fileName: String = "csca.ks",
        password: String = ""
    ): Uri? {
        ...
        if (!outputDir.exists()) {
            Log.d(
                "",
                "DEBUG: output dir " + outputDir.path.toString() + " doesn't exist, creating it."
            )
            if (!outputDir.mkdirs()) {
    ...
    fun getCountry(principal: X500Principal): Country? {
        ...
        val countryCode =
            issuerName.substring(startIndex + 2, endIndex).trim { it <= ' ' }.toUpperCase()
        return try {
    ...
    fun toMap(certificates: List<Certificate>): Map<String, X509Certificate> {
        ...
        for (certificate in certificates) {
            ...
            val outName = country!!.toAlpha2Code().toLowerCase()
                .toString() + "_" + (if (isSelfSigned) "root_" else "link_") + (++i) + ".cer"
