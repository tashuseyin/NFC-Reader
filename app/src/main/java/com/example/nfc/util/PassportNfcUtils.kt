...
import ercanduman.cardreader.utils.EACCredentials
...
object PassportNfcUtils {
    ...
    fun retrieveSignatureImage(context: Context, dg7File: DG7File): Bitmap {
        ...
        if (!displayedImageInfos.isEmpty()) {
            ...
            return toBitmap(
                displayedImageInfo.imageLength,
                displayedImageInfo.imageInputStream,
                displayedImageInfo.mimeType
            )
        }
        throw IOException("Unable to decodeImage Image")
    }
    ...
    fun retrieveFingerPrintImage(context: Context, dg3File: DG3File): List<Bitmap> {
        ...
        while (iterator.hasNext()) {
            ...
            val bitmap = toBitmap(
                fingerImageInfo.imageLength,
                fingerImageInfo.imageInputStream,
                fingerImageInfo.mimeType
            )
    ...
    fun getCertificateChain(
        ...
        try {
            ...
            val docStoreParams =
                CollectionCertStoreParameters(setOf<Certificate>(docSigningCertificate as Certificate))
