package ercanduman.cardreader.utils
...
private constructor() {
    ...
    fun setEACPublicKey(eacPublicKey: PublicKey) {
        ...
        val dg14File =
            DG14File(Arrays.asList(*arrayOf<SecurityInfo>(chipAuthenticationPublicKeyInfo)))
    ...
    private fun verifyAA() {
        ...
        try {
            ...
            val aaResult = (service as PassportService).doAA(
                dg15File!!.publicKey,
            ...
            Log.e(
                TAG,
                "DEBUG: this exception wasn't caught in verification logic (< 0.4.8) -- MO 3. Type is " + e.javaClass.canonicalName!!
            )
            e.printStackTrace()
            verificationStatus.setAA(
                VerificationStatus.Verdict.FAILED,
    ...
    private fun verifyAA(
        ...
        try {
            ...
            if ("RSA" == pubKeyAlgorithm) {
                ...
                rsaAADigest =
                    MessageDigest.getInstance(digestAlgorithm) /* NOTE: for output length measurement only. -- MO */
                ...
                val digestLength =
                    rsaAADigest!!.digestLength /* SHA1 should be 20 bytes = 160 bits */
    ...
    private fun checkDocSignature(docSigningCert: Certificate?): Boolean {
        ...
        if (digestEncryptionAlgorithm.endsWith("withRSA/PSS")) {
            val saltLength = findSaltRSA_PSS(
